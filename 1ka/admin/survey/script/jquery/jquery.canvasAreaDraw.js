(function( $ ){
  ...
  var init = function(index, input, options) {
	...
	$reset = $('<button type="button" class="btn"><i class="icon-trash"></i>'+settings.clearButton+'</button>');
    $canvas = $('<canvas>');
...
/*     resize = function() {
	...
	resize = function() {
		if(settings.imageHeight != 0 || settings.imageWidth != 0){
			...
			$canvas.attr('height', settings.imageHeight);
			$canvas.attr('width', settings.imageWidth);
		}
		else{
			$canvas.attr('height', image.height).attr('width', image.width);
		}
	...
	$canvas.attr('class', 'hotspot_canvas');
	
	if(settings.imageHeight != 0 || settings.imageWidth != 0){
		$canvas.css('background-size', settings.imageWidth+'px '+settings.imageHeight+'px');
	}
    ...
    reset = function() {
	  //pokazi skrij hotspot_tips	  
	  $('#hotspot_tips_'+settings.spr_id).css('display', 'none');	 
      points = [];
      draw();
    };
	
	show = function() {
      //points = [settings.imageCoords];
	  //points = [];
	  points = settings.imageCoords;
	  points = points.split(",");
      //draw();
	  ...
	  //var coords = settings.imageCoords;
	  //console.log("Points: "+coords[2]);
    };
    ...
    mousedown = function(e) {
	  //pokazi skriti hotspot_tips	  
	  $('#hotspot_tips_'+settings.spr_id).css('display', '');	  
