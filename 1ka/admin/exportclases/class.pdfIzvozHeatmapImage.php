...
class PdfIzvozHeatmapImage {
	...
	*/
	...
	function createPdf(){		
		...
		$this->pdf->AddPage();
 		...
 		if($this->frontpage != 1){
			...
			$this->pdf->MultiCell(150, 5, $lang['export_analysis_heatmap_image'], 0, 'L', 0, 1, 0 ,0, true);
			$this->pdf->setFont('','','7');
	...
	function displayImage($spremenljivka){
		...
		//$this->pdf->Image($image, $x='', $y='', $w=140, $h, $type='PNG', $link='', $align='N', $resize=true, $dpi=1600, $palign='C', $ismask=false, $imgmask=false, $border=1);		
		$this->pdf->Image($image, $x='', $y='', $w=140, $h, $type='PNG');		
	}	
