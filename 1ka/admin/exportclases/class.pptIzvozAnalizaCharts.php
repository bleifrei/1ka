...
class PptIzvozAnalizaCharts {
	...
	*/
	...
	// Vstavimo graf
	function displayChart($spid, $type=0, $sort=0, $value_type=0){
		global $site_path;
		global $lang;
		...
		if(isset($this->sessionData[$spid])){
			...
			if(count(SurveyAnalysis::$_LOOPS) == 0)
				...
				$settings = $this->sessionData[$spid][SurveyAnalysis::$_CURRENT_LOOP['cnt']];
		...
		$shape->setPath('../survey/pChart/Cache/'.$imgName);
		$shape->setWidth(800);
		//$shape->setHeight(400);
		$shape->setOffsetX(75);
