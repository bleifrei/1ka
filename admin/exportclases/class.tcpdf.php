		...
		*/
		public function getNumLines($txt, $w=0) {
			...
			foreach ($txtblocks as $block) {
				...
				$string = explode(" ", $block);
				$sum = 0;
				$lines = 1;
				foreach($string as $word){
					
					if($sum > 0){
						if($this->GetStringWidth($word) + $sum < $wmax)
							$sum += $this->GetStringWidth($word);
						else{
							$sum = $this->GetStringWidth($word);
							$lines++;
						}
						...
						if($this->GetStringWidth($word) < $wmax)
							$sum = $this->GetStringWidth($word);
						else{
							$lines++;
						}
			...
			switch($tag['value']) {
				...
				case 'img': {
					if (isset($tag['attribute']['src'])) {
						...
						// S tem ne delajo slike vkljucene v html, ki so na streznik uploadane
