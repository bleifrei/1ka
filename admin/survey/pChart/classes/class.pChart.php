 ...
 class pChart
  {
   ...
   // Skrajsa text na zeljeno dolzino
   function snippet($text,$length=30,$tail="...")
	{
		$text = trim($text);
		$txtl = strlen($text);
		if($txtl > $length)
		{
			for($i=1;$text[$length-$i]!=" ";$i++)
			{
				if($i == $length)
				{
					return substr($text,0,$length-2) . $tail;
				}
			}
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
   ...
   function drawScale($Data,$DataDescription,$ScaleMode,$R,$G,$B,$DrawTicks=TRUE,$Angle=0,$Decimals=1,$WithMargin=FALSE,$SkipLabels=1,$RightScale=FALSE, $roundText=30)
    {
     ...
     if ( $this->VMin == NULL && $this->VMax == NULL)
      {
       ...
       elseif ( $ScaleMode == SCALE_ADDALL || $ScaleMode == SCALE_ADDALLSTART0 ) /* Experimental */
		...
		// Zaradi zaokrozevanja je tukaj prebil 100% in je narisal preveliko skalo (33.33 + 66.67 = 100.01 = 101)
         ...
         while(!$ScaleOk)
          {
           ...
           if (!$ScaleOk)
			...
			// Dodaten pogoj da ne delimo slucajno z 0
			if($Factor == 0){				
				$Factor = $oldFactor;
     ...
     for($i=1;$i<=$Divisions+1;$i++)
      {
       if ( $RightScale )
        $this->drawLine($this->GArea_X2,$YPos,$this->GArea_X2+floor($this->FontSize/2),$YPos,$R,$G,$B);
       else
        $this->drawLine($this->GArea_X1,$YPos,$this->GArea_X1-floor($this->FontSize/2),$YPos,$R,$G,$B);
       ...
       if ( $RightScale )
        {
         imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X2+$this->FontSize,$YPos+($this->FontSize/2),$C_TextColor,$this->FontName,$Value);
         if ( $XMin < $this->GArea_X2+15+$TextWidth || $XMin == NULL ) { $XMin = $this->GArea_X2+15+$TextWidth; }
        }
       else
        {
         imagettftext($this->Picture,$this->FontSize,0,$this->GArea_X1-$this->FontSize-$TextWidth,$YPos+($this->FontSize/2),$C_TextColor,$this->FontName,$Value);
         if ( $XMin > $this->GArea_X1-$this->FontSize-$TextWidth || $XMin == NULL ) { $XMin = $this->GArea_X1-$this->FontSize-$TextWidth; }
        }
     ...
     foreach ( $Data as $Key => $Values )
      {
       if ( $ID % $SkipLabels == 0 )
        {
         $this->drawLine(floor($XPos),$this->GArea_Y2,floor($XPos),$this->GArea_Y2+floor($this->FontSize/2),$R,$G,$B);
         ...
         if ( $Angle == 0 )
          {
           $YPos = $this->GArea_Y2+(2*$this->FontSize);
           imagettftext($this->Picture,$this->FontSize,$Angle,floor($XPos)-floor($TextWidth/2),$YPos,$C_TextColor,$this->FontName,$Value);
          }
         else
          {
           $YPos = $this->GArea_Y2+(1.5*$this->FontSize)+$TextHeight;
           if ( $Angle <= 90 )
            imagettftext($this->Picture,$this->FontSize,$Angle,floor($XPos)-$TextWidth+floor($this->FontSize/2),$YPos,$C_TextColor,$this->FontName,$Value);
           else
            imagettftext($this->Picture,$this->FontSize,$Angle,floor($XPos)+$TextWidth+floor($this->FontSize/2),$YPos,$C_TextColor,$this->FontName,$Value);
          }
    ...
    if ( isset($DataDescription["Axis"]["X"]) )
      {
       ...
       imagettftext($this->Picture,$this->FontSize,0,$TextLeft,$YMax+$this->FontSize+floor($this->FontSize/2),$C_TextColor,$this->FontName,$DataDescription["Axis"]["X"]);
      }
    }
   ...
   function drawLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=false,$reverse=false)
    {
     /* Validate the Data and DataDescription array */
	 ...
	 $TextHeight = $this->FontSize*1.4;
	 
     /* <-10->[8]<-4->Text<-10-> */
     $MaxWidth = 0; $MaxHeight = 8;
     foreach($DataDescription["Description"] as $Key => $Value)
      {
	   $Value = $this->snippet($Value, 12);
     ...
     foreach($DataDescription["Description"] as $Key => $Value)
      {
       ...
       $this->drawFilledRectangle($XPos+10,$YPos+$YOffset-2-floor(0.8*$this->FontSize),$XPos+10+floor(0.8*$this->FontSize),$YPos+$YOffset-2,$this->Palette[$ID]["R"],$this->Palette[$ID]["G"],$this->Palette[$ID]["B"]);
       imagettftext($this->Picture,$this->FontSize,0,$XPos+($this->FontSize*2)+7,$YPos+$YOffset,$C_TextColor,$this->FontName,$Value);
   ...
   /* Narisemo posebno legendo pri povprecjih multigridov */
   function drawAvgLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=false,$reverse=false)
    {
     ...
     foreach($DataDescription as $Key => $Value)
      {
       ...
       //$this->drawFilledRectangle($XPos+10,$YPos+$YOffset-2-floor(0.8*$this->FontSize),$XPos+10+floor(0.8*$this->FontSize),$YPos+$YOffset-2,$this->Palette[$ID]["R"],$this->Palette[$ID]["G"],$this->Palette[$ID]["B"]);
       imagettftext($this->Picture,$this->FontSize,0,$XPos+($this->FontSize),$YPos+$YOffset,$C_TextColor,$this->FontName,$Value);
	...
	/* Izrisemo vertikalno legendo */
   function drawVerticalLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=false)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("drawLegend",$DataDescription);
     ...
     foreach($DataDescription["Description"] as $Key => $Value)
      {
	   $Value = $this->snippet($Value, 30);
	  
       $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Value);
       $TextWidth  = $Position[2]-$Position[0];
       $TextHeight = $Position[1]-$Position[7];

	    // Ce je legenda presiroka jo razbijemo v 2 vrstici
		if(($this->FontSize * 80) < $width){
			$width2 += $TextWidth + ($this->FontSize*4);		
		}
		else{
			$width += $TextWidth + ($this->FontSize*4);
			$split_counter++;
		}
      }	  
	  
	  $max_width = ($width2 > $width) ? $width2 : $width;
	  $XPos2 = $XPos - round($max_width/2);
     ...
     if ( $Border )
      {
		// legenda v 1 vrstici
		if($width2 == 0){
			$this->drawFilledRoundedRectangle($XPos2+1,$YPos+1,$XPos2+$max_width+1,$YPos+20+1,5,$Rs,$Gs,$Bs);
			$this->drawFilledRoundedRectangle($XPos2,$YPos,$XPos2+$max_width,$YPos+20,5,$R,$G,$B);
		}
		// legenda v 2 vrsticah
		else{
			$this->drawFilledRoundedRectangle($XPos2+1,$YPos+1,$XPos2+$max_width+1,$YPos+20+1,5,$Rs,$Gs,$Bs);
			$this->drawFilledRoundedRectangle($XPos2,$YPos,$XPos2+$max_width,$YPos+40,5,$R,$G,$B);
		}      
      }
		 ...
		 foreach($DataDescription["Description"] as $Key => $Value)
		  {
		   ...
		   $this->drawFilledRectangle($XPos2+10+$XOffset,$YPos+11-floor(0.8*$this->FontSize/2),$XPos2+10+$XOffset+floor(0.8*$this->FontSize),$YPos+11+floor(0.8*$this->FontSize/2),$this->Palette[$ID]["R"],$this->Palette[$ID]["G"],$this->Palette[$ID]["B"]);
		   imagettftext($this->Picture,$this->FontSize,0,$XPos2+($this->FontSize*2)+7+$XOffset,$YPos+11+floor($this->FontSize/2),$C_TextColor,$this->FontName,$Value);

		   $Position   = imageftbbox($this->FontSize,0,$this->FontName,$Value);
		   $TextWidth  = $Position[2]-$Position[0] + ($this->FontSize*4);
		 ...
		 foreach($DataDescription["Description"] as $Key => $Value)
		  {
		   ...
		   // izrisujemo 2. vrstico
		   if($ID >= $split_counter){
				$XPos2 = $XPos - round($width2/2);
				if($ID == $split_counter)
					$YPos += $TextHeight + 10;
		   ...
		   // izrisujemo 1. vrstico
		   else{	
				$XPos2 = $XPos - round($width/2);
	...
	/* Izrisemo vertikalno legendo */
   function drawAvgVerticalLegend($XPos,$YPos,$DataDescription,$R,$G,$B,$Rs=-1,$Gs=-1,$Bs=-1,$Rt=0,$Gt=0,$Bt=0,$Border=false)
    {
     if ( !isset($DataDescription) )
      return(-1);
		 ...
		 foreach($DataDescription as $Key => $Value)
		  {
		   ...
		   //$this->drawFilledRectangle($XPos2+10+$XOffset,$YPos+11-floor(0.8*$this->FontSize/2),$XPos2+10+$XOffset+floor(0.8*$this->FontSize),$YPos+11+floor(0.8*$this->FontSize/2),$this->Palette[$ID]["R"],$this->Palette[$ID]["G"],$this->Palette[$ID]["B"]);
		   imagettftext($this->Picture,$this->FontSize,0,$XPos2+($this->FontSize)+$XOffset,$YPos+11+floor($this->FontSize/2),$C_TextColor,$this->FontName,$Value);
   ...
   /* Draw the data legends */
   function drawPieLegend($XPos,$YPos,$Data,$DataDescription,$R,$G,$B,$Border=false)
    {
     /* Validate the Data and DataDescription array */
     ...
     foreach($Data as $Key => $Value)
      {
       $Value2 = $this->snippet($Value[$DataDescription["Position"]], 28);
     ...
     foreach($Data as $Key => $Value)
      {
       ...
       imagettftext($this->Picture,$this->FontSize,0,$XPos+($this->FontSize*2)+7,$YPos+$YOffset,$C_TextColor,$this->FontName,$Value2);
       $YOffset = $YOffset + $TextHeight + 4;
       $ID++;
      }
    }
     ...
     foreach ( $DataDescription["Values"] as $Key2 => $ColName )
      {
       if ( $SerieName == "" || $SerieName == $ColName )
        {
         $XIn = array(); $Yin = array(); $Yt = array(); $U = array();
         $XIn[0] = 0; $YIn[0] = 0;
   ...
   /* This function draw a bar graph */
   function drawBarGraph($Data,$DataDescription,$Shadow=FALSE,$Alpha=100,$insideValues=false,$insideValuesSmall=false)
    {
     /* Validate the Data and DataDescription array */
			 ...
			 if($insideValues == true){
			   ...
			   $Y = ($YZero+1 + $YPos-1) / 2;
				...
				elseif($insideValuesSmall == true){
					$C_TextColor = $this->AllocateColor($this->Picture,0,0,0);			
					imagettftext($this->Picture,$this->FontSize,0,$X-($TextWidth/2),$YPos-4,$C_TextColor,$this->FontName,$Text);
				}
			 }
			}
          }
   ...
   function drawStackedBarGraph($Data,$DataDescription,$insideValues=false,$Alpha=50,$Contiguous=FALSE)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("drawBarGraph",$DataDescription);
     $this->validateData("drawBarGraph",$Data);
           ...
           if ( is_numeric($Data[$Key][$ColName]) )
            {
			 ...
			 if($insideValues == true){
			   ...
			   $X = ($XPos + $XPos + $SeriesWidth) / 2;
			   $Y = ($YBottom+1 + $YPos-1) / 2;

				$text_colors = $this->getContrastTextColor($this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
				$C_TextColor = $this->AllocateColor($this->Picture,$text_colors['R'],$text_colors['G'],$text_colors['B']);
			   
				if($Value > 0 && ($YBottom-$YPos > 15))
					imagettftext($this->Picture,$this->FontSize,0,$X-($TextWidth/2),$Y+4,$C_TextColor,$this->FontName,$Text);		 
			 }
   ...
   function drawRadarAxis($Data,$DataDescription,$Mosaic=TRUE,$BorderOffset=10,$A_R=60,$A_G=60,$A_B=60,$S_R=200,$S_G=200,$S_B=200,$MaxValue=-1, $ScalePosition=0)
    {
     /* Validate the Data and DataDescription array */
     ...
     if ( $MaxValue == -1 )
	  ...
	  // Zascita pred praznimi vprasanji brez podatkov
	  if($MaxValue == 0)
		return;  
	 ...
	 // Nastavimo interval in zacetek skale - zaenkrat imamo vedno 3 vrednosti
	 // Imamo povprecja - zacnemo skalo od 1 do st vseh variabel
	 if($this->VMin == 1){
		$start = 1;
		$interval = 1;
	 }
	 else{
		$interval = floor($MaxValue / 2);
		$start = 0;
	 }
	 /*if($MaxValue > 50){
		$interval = 20;
	 ...
	 if($interval == 0){
		 $interval = (round($MaxValue / 2, 1) == 0) ? 1 : round($MaxValue / 2, 1);
	 }
	 
	 /* Draw the spider web */
     for ( $t=1; $t<=$MaxValue; $t+=$interval)
      {
     ...
     for ( $i=0; $i<=$Points; $i++)
      {
	   ...
	   $XLabel = cos($Angle * 3.1418 / 180 ) * ($Radius + 5) + $XCenter;
       $YLabel = sin($Angle * 3.1418 / 180 ) * ($Radius + 5) + $YCenter;
       $XOffset = 0; $YOffset = 0;
       if (isset($Data[$i][$DataDescription["Position"]]))
        {
         $Label = $Data[$i][$DataDescription["Position"]];
		 // skrcimo text na max dolzino
		 if(strlen($Label) > 45){		 
			$Label = $this->snippet($Label, 42);
		 }	 
         ...
         imagettftext($this->Picture,$this->FontSize,0,$XLabel+$XOffset,$YLabel+$YOffset,$C_TextColor,$this->FontName,$Label);
        }
      }

     /* Write the values */	 
     for ( $t=$start; $t<=$MaxValue+$this->VMin; $t+=$interval)
      {
       // postavitev label na os
	   if($ScalePosition == 0){
		   $TRadius = ( $Radius / $MaxValue ) * ($t-$this->VMin);
	   ...
	   // postavitev label na diagonalo (originalno)
	   else{
		   $TRadius = ( $Radius / $MaxValue ) * ($t-$this->VMin);
       ...
       foreach ( $Data as $Key => $Values )
        {
         if ( isset($Data[$Key][$ColName]))
          {
           $Value    = $Data[$Key][$ColName];
           $Strength = ( $Radius / $MaxValue ) * ($Value-$this->VMin);
           ...
           if ( $XLast != -1 ){
			...
			// Naredimo piko na koncu crte
			$this->drawFilledCircle($XPos+1,$YPos+1,3*floor($this->FontSize/8),$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
			$this->drawFilledCircle($XLast+1,$YLast+1,3*floor($this->FontSize/8),$this->Palette[$ColorID]["R"],$this->Palette[$ColorID]["G"],$this->Palette[$ColorID]["B"]);
		   }
       ...
       if ( !($DrawLabels == PIE_NOLABEL) )
        {
		 ...
		 $result = round($Value * pow(10,$Decimals) * $SplicePercent)/pow(10,$Decimals);
         ...
         elseif ($DrawLabels == PIE_PERCENTAGE_LABEL)
          ...
          elseif ($DrawLabels == 'custom' && $result > 4)
          $Caption  = $iLabels[$Key]." (".(round($Value * pow(10,$Decimals))/pow(10,$Decimals)).")";
		 elseif ($DrawLabels == 'custom_percent' && $result > 4)
          $Caption  = $iLabels[$Key]." (".$result."%)";
   ...
   /* This function draw a flat pie chart */
   function drawFlatPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$SpliceDistance=0,$Decimals=0,$AllBlack=FALSE,$specialLabels=false)
    {
     /* Validate the Data and DataDescription array */
     ...
     $SpliceRatio   = ($PieSum == 0) ? 360 : (360 / $PieSum);
     $SplicePercent = ($PieSum == 0) ? 100 : (100 / $PieSum);
	 
	 // obrnemo vrstni red
	 //$iValues = array_reverse($iValues);
	 //$iLabels = array_reverse($iLabels); 
     ...
     foreach($iValues as $Key => $Value)
      {
       ...
       if ( !($DrawLabels == PIE_NOLABEL) )
        {
		 // skrcimo text na max dolzino
		 if(strlen($iLabels[$Key]) > 40){
			$iLabels[$Key] = $this->snippet($iLabels[$Key], 40);
		 }	 
         ...
         elseif ($DrawLabels == PIE_PERCENTAGE_LABEL)
          $Caption  = $iLabels[$Key]."\r\n".(round($Value * pow(10,$Decimals) * $SplicePercent)/pow(10,$Decimals))."%";
		 elseif ( ($DrawLabels == 'custom' || $DrawLabels == 'custom_sort') && $result > 4)
          $Caption  = $iLabels[$Key]." (".(round($Value * pow(10,$Decimals))/pow(10,$Decimals)).")";
		 elseif ( ($DrawLabels == 'custom_percent' || $DrawLabels == 'custom_percent_sort') && $result > 4)
          $Caption  = $iLabels[$Key]." (".$result."%)";
         ...
         if ( $TAngle > 0 && $TAngle < 180 )
          $TY = sin(($TAngle) * 3.1418 / 180 ) * ($Radius+10+$SpliceDistance) + $YPos + $this->FontSize;
         else
         ...
         imagettftext($this->Picture,$this->FontSize,0,$TX,$TY,$C_TextColor,$this->FontName,$Caption);
			...
			// Dodatne labele s crto (pod 4%)
			if (($DrawLabels == 'custom_percent_sort' || $DrawLabels == 'custom_sort') && $result <= 4){
				$Caption  = ($DrawLabels == 'custom_percent_sort') ? $iLabels[$Key]." (".$result."%)" : $iLabels[$Key]." (".(round($Value * pow(10,$Decimals))/pow(10,$Decimals)).")";

				// ZACETEK POSEVNE CRTE
				$TX = cos(($TAngle) * 3.1418 / 180 ) * ($Radius+$SpliceDistance) + $XPos;
				
				if ( $TAngle > 0 && $TAngle < 180 )
					$TY = sin(($TAngle) * 3.1418 / 180 ) * ($Radius+$SpliceDistance) + $YPos + 4;
				else
					$TY = sin(($TAngle) * 3.1418 / 180 ) * ($Radius+$SpliceDistance) + $YPos - ($TextHeight/2);
				
				if ( $TAngle > 90 && $TAngle < 270 )
					$TX = $TX - $TextWidth;

				// IZRACUN NAKLONA CRTE
				//$lineAngle = ($lineAngle == -1) ? 50+$TAngle : $lineAngle-13;
				/*if($lineAngle == -1)
					$lineAngle = (abs(sin($TAngle)) > 0.2) ? -$TAngle - 13 : -$TAngle;
				else
					$lineAngle -= 13;

				$length = 10 + abs(sin(($lineAngle) * 3.1418 / 180)) * 60;

				//KONEC POSEVNE CRTE
				$PosX = cos(($lineAngle) * 3.1418 / 180 ) * $length + $TX;
				$PosY = $TY - 4 - sin(($lineAngle) * 3.1418 / 180 ) * $length;*/
				
				$scaling = round($Radius / 95);
				
				$length = 40*$scaling;
				$PosX = $length + $TX;
				$PosY = ($TY-$globalY > 58*$scaling) ? $TY-(40*$scaling) : $globalY+(18*$scaling);
				
				//IZRIS POSEVNE CRTE
				$this->drawLine($TX,$TY,$PosX,$PosY,$Rc,$Gc,$Bc);
				
				//VODORAVNA CRTA
				$TX = $PosX + 10;
				$TY = $PosY;		
				$this->drawLine($PosX,$PosY,$TX,$TY,$Rc,$Gc,$Bc);
			...
			// nastavimo visino prejsnje labele
			else
				$globalY = $TY;
        }
   ...
   function drawPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$EnhanceColors=TRUE,$Skew=60,$SpliceHeight=20,$SpliceDistance=0,$Decimals=0, $roundText=30)
    {
     /* Validate the Data and DataDescription array */
       ...
       if ( !($DrawLabels == PIE_NOLABEL) )
        {
		 ...
		 elseif ( ($DrawLabels == 'custom_percent' || $DrawLabels == 'custom_percent_sort') && $result > 4)
		  ...
		  $Caption = $this->snippet($Caption,$roundText);
         ...
         if ( $TAngle > 0 && $TAngle < 180 )
          $TY = sin(($TAngle) * 3.1418 / 180 ) * ($SkewHeight + 10) + $YPos + $SpliceHeight + $this->FontSize;
         else
         ...
         $C_TextColor = $this->AllocateColor($this->Picture,20,20,20);
         imagettftext($this->Picture,$this->FontSize,0,$TX,$TY,$C_TextColor,$this->FontName,$Caption);
        }
   ...
   function drawFilledRectangle($X1,$Y1,$X2,$Y2,$R,$G,$B,$DrawBorder=TRUE,$Alpha=100,$NoFallBack=FALSE)
    {
     ...
     if ( $Alpha == 100 )
      {
       /* Process shadows - Preskakujemo zaradi optimizacije */
   ...
   function drawAntialiasPixel($X,$Y,$R,$G,$B,$Alpha=100,$NoFallBack=FALSE)
    {
     ...
     $Yi   = floor($Y);
	  ...
	  // Pri pie in radarju nastavimo aa ker zgleda drugace slabo
   ...
   function getContrastTextColor($R, $G, $B) {
		...
		// formula za izracun svetlosti barve (255->max svetla, 0->max temna)
		...
		$text_colors = array('R'=>$newR, 'G'=>$newG, 'B'=>$newB);
		return $text_colors;
	}
  ...
  /* zaokrozimo stevilo $number na $toNearest */
	...
	function setAntialias($use, $quality){
